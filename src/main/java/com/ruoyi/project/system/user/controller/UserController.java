...
public class UserController extends BaseController
{
    ...
    public AjaxResult editSave(@Validated User user)
    {
        ...
        boolean hasKey = redisService.hasKey(Constants.TOKEN_KEY_FRIST + user.getLoginName() + "_info");
        if(hasKey){
        	redisService.delete(Constants.TOKEN_KEY_FRIST + user.getLoginName() + "_info");
        }
    ...
    public AjaxResult remove(String ids)
    {
        try
        {
        	List<PageData> userList = userService.selectUserByIds(ids);
        	int result = userService.deleteUserByIds(ids);
        	if(userList != null && userList.size() > 0){
        		for (PageData pageData : userList) {
        			boolean hasKey = redisService.hasKey(Constants.TOKEN_KEY_FRIST + pageData.getString("login_name") + "_info");
                    if(hasKey){
                    	redisService.delete(Constants.TOKEN_KEY_FRIST + pageData.getString("login_name") + "_info");
                    }
				}
        	}
    ...
    public AjaxResult changeStatus(User user)
    {
    	...
    	String id = userId + "";
    	List<PageData> userList = userService.selectUserByIds(id);
    	if(userList != null && userList.size() > 0){
    		for (PageData pageData : userList) {
    			boolean hasKey = redisService.hasKey(Constants.TOKEN_KEY_FRIST + pageData.getString("login_name") + "_info");
                if(hasKey){
